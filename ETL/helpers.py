...
def flatten_response(response_from_api) -> str:
    '''
...
def get_base_stats_table_data_from_api_response(api_response):
    '''
    Gets the relevant data for the base_stats table from the API response
    '''
    api_response=flatten_response(api_response)
    base_stats_data=(api_response['date'], api_response['states'],api_response['cases_total'],api_response['testing_total'])
    return base_stats_data



def write_to_base_stats_table(base_stats_row):
    '''
...
def get_outcome_stats_table_data_from_api_response(api_response):
    '''
    Gets the relevant data for the outcome_stats table from the API response
    '''
    api_response=flatten_response(api_response)
    outcome_stats_data=(api_response['outcomes_hospitalized_currently'], api_response['outcomes_hospitalized_in_icu_currently'],api_response['outcomes_hospitalized_on_ventilator_currently'],api_response['outcomes_death_total'],api_response['date'])
    return outcome_stats_data

def write_to_outcome_stats_table(outcome_stats_row):
    '''
...
def extract_data_from_api_and_load_to_database(date):
    '''
